...
(function () {
  ...
  function sendHeight(force, delay) {
    ...
    heightTimer = setTimeout(function () {
      var height = Math.ceil(pageHeight());
  ...
  function requestParentScroll(target, navHeight) {
    if (!target) return;

    send({
      source: CHILD_SOURCE,
      type: 'SYURA_PARENT_SCROLL_TO',
      targetY: target.getBoundingClientRect().top + (window.scrollY || 0),
  ...
  function youtubeId(url) {
    ...
    var match = value.match(/youtu\.be\/([a-zA-Z0-9_-]{6,})/);
    if (match) return match[1];

    match = value.match(/[?&]v=([a-zA-Z0-9_-]{6,})/);
    if (match) return match[1];

    match = value.match(/\/embed\/([a-zA-Z0-9_-]{6,})/);
    if (match) return match[1];
  ...
  function openPortfolioCard(card) {
    if (!card) return;

    var href = String(card.getAttribute('href') || '').trim();
    var title = cardText(card, '.pfTitle') || '포트폴리오';
    var desc = cardText(card, '.pfDesc');
    var yt = youtubeId(href);
  ...
  function bindClicks() {
    if (window.__ayoniBridgeV5Clicks) return;
    window.__ayoniBridgeV5Clicks = true;

    document.addEventListener('click', function (event) {
      var jump = event.target.closest('.pfJump');
      if (jump) {
        var href = String(jump.getAttribute('href') || '').trim();

        if (href.charAt(0) === '#') {
          var jumpTarget = document.querySelector(href);

          if (jumpTarget) {
            event.preventDefault();
            event.stopPropagation();
            requestParentScroll(jumpTarget, 66);
          }
        }
        return;
      }
      ...
      var topLink = event.target.closest('.topnav__links a[name="goto"]');
      if (topLink) {
        var targetId = String(topLink.getAttribute('data-goto') || '').trim();
        var section = targetId ? document.getElementById(targetId) : null;

        if (section) {
          event.preventDefault();
          event.stopPropagation();
          requestParentScroll(section, 66);
        }
      }
    }, true);
  ...
  function bindParentMessages() {
    if (window.__ayoniBridgeV5Messages) return;
    window.__ayoniBridgeV5Messages = true;

    window.addEventListener('message', function (event) {
      var data = event.data || {};
      if (data.source !== PARENT_SOURCE) return;

      if (data.type === 'AYONI_PARENT_NAV_TO') {
        var target = document.getElementById(String(data.targetId || '').trim());
        if (target) requestParentScroll(target, 66);
      }
    });
  }

  function bindHeight() {
    if (window.__ayoniBridgeV5Height) return;
    window.__ayoniBridgeV5Height = true;
    ...
    window.addEventListener('load', function () {
      sendHeight(true, 20);
      setTimeout(function () { sendHeight(true, 0); }, 250);
      setTimeout(function () { sendHeight(true, 0); }, 800);
      setTimeout(function () { sendHeight(true, 0); }, 1800);
    });
    ...
    document.addEventListener('click', function () {
      setTimeout(function () { sendHeight(true, 0); }, 160);
    }, true);
  ...
  function boot() {
    ...
    sendHeight(true, 20);
    setTimeout(function () { sendHeight(true, 0); }, 400);
    setTimeout(function () { sendHeight(true, 0); }, 1200);
  }
